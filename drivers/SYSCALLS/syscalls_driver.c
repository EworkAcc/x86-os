...
static int syscall_write(struct cpu_state *cpu) {
  ...
  if(fd == 1 || fd == 2) { klog_write((const char *)cpu->ecx, cpu->edx); return (int)cpu->edx; }
...
static int syscall_exit(__attribute__((unused)) struct cpu_state *cpu) { klog_write_string("user program exit\n"); for(;;) asm volatile ("cli; hlt"); return 0; }
